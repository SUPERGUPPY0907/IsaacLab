...
TRUST_REGION_MODE_SWEEP="${TRUST_REGION_MODE_SWEEP:-ppo,spo,aspo}"
FIXED_LR_SWEEP="${FIXED_LR_SWEEP:-5e-5,1e-4,5e-4}"

HEADLESS="${HEADLESS:-1}"
NUM_ENVS="${NUM_ENVS:-}"
MAX_ITERATIONS="${MAX_ITERATIONS:-}"
SAVE_INTERVAL="${SAVE_INTERVAL:-}"
EXPERIMENT_PREFIX="${EXPERIMENT_PREFIX:-g1_fpo_sweep}"
LAUNCH_LOG_DIR="${LAUNCH_LOG_DIR:-${ISAACLAB_ROOT}/logs/g1_fpo_sweeps}"
GPU_IDS="${GPU_IDS:-}"
...
Environment overrides:
  ...
  AGENT                     G1 FPO agent key. Default: fpo
                            Supported: fpo, fpo_pp, fpo++
  SEEDS                     Comma-separated seeds. Default: 42
  SCHEDULE_SWEEP            Comma-separated LR schedule modes. Default: adaptive,fixed
  TRUST_REGION_MODE_SWEEP   Comma-separated trust-region modes. Default: ppo,spo,aspo
  FIXED_LR_SWEEP            Comma-separated learning rates used only when schedule=fixed.
                            Default: 5e-5,1e-4,5e-4
  ...
  EXPERIMENT_PREFIX         Prefix added to generated experiment_name values. Default: g1_fpo_sweep
  LAUNCH_LOG_DIR            Directory for per-run launcher logs. Default: logs/g1_fpo_sweeps
  ...
  WANDB_PROJECT             Optional override. Default:
                            <TASK>_fpo_lr_schedule_trust_region_mode_sweep

Examples:
  bash ${0##*/}
  TRUST_REGION_MODE_SWEEP=ppo,aspo bash ${0##*/}
  SCHEDULE_SWEEP=fixed FIXED_LR_SWEEP=1e-4,5e-4 GPU_IDS=0,1 bash ${0##*/}
EOF
}
...
validate_agent() {
    case "${AGENT}" in
        fpo | fpo_pp | fpo++)
            ;;
        *)
            echo "AGENT='${AGENT}' is not a supported FPO entry point for this sweep." >&2
            echo "Use one of: fpo, fpo_pp, fpo++" >&2
            exit 1
            ;;
    esac
}
...
build_wandb_project() {
    ...
    if [[ -n "${WANDB_PROJECT}" ]]; then
        ...
        echo "${task}_fpo_lr_schedule_trust_region_mode_sweep"
    fi
}
...
build_variant_name() {
    ...
    local variant="trust$(sanitize_value "${trust_region_mode}")_sched$(sanitize_value "${schedule}")"
    if [[ "${schedule}" == "adaptive" ]]; then
        variant="${variant}_lrcfg"
    else
        variant="${variant}_lr$(sanitize_value "${learning_rate}")"
    fi
    echo "${variant}"
}
...
schedule_variant() {
    ...
    local seed="$3"
    local trust_region_mode="$4"
    local schedule="$5"
    local learning_rate="${6:-}"

    local variant
    variant="$(build_variant_name "${trust_region_mode}" "${schedule}" "${learning_rate}")"
...
if [[ ${#seeds[@]} -eq 0 || ${#schedule_modes[@]} -eq 0 || ${#trust_region_modes[@]} -eq 0 ]]; then
    echo "SEEDS, SCHEDULE_SWEEP, and TRUST_REGION_MODE_SWEEP must all be non-empty." >&2
    exit 1
fi
...
for seed in "${seeds[@]}"; do
    ...
    for trust_region_mode in "${trust_region_modes[@]}"; do
        ...
        for schedule in "${schedule_modes[@]}"; do
            ...
            case "${schedule}" in
                adaptive)
                    schedule_variant "${TASK}" "${wandb_project}" "${seed}" "${trust_region_mode}" "adaptive"
                    ;;
                fixed)
                    ...
                    for learning_rate in "${fixed_lr_values[@]}"; do
                        ...
                        schedule_variant "${TASK}" "${wandb_project}" "${seed}" "${trust_region_mode}" "fixed" "${learning_rate}"
                    done
                    ;;
                *)
...
if [[ ${failed_jobs} -ne 0 ]]; then
    echo "G1 FPO trust-region sweep finished with ${failed_jobs} failed job(s)." >&2
    exit 1
fi
echo "G1 FPO trust-region sweep finished successfully."
