...
ALGORITHMS="${ALGORITHMS:-genpo,belm_genpo}"
...
EXPERIMENT_PREFIX="${EXPERIMENT_PREFIX:-genpo_belm_genpo_10seed}"
WANDB_PROJECT_PREFIX="${WANDB_PROJECT_PREFIX:-}"
LAUNCH_LOG_DIR="${LAUNCH_LOG_DIR:-${ISAACLAB_ROOT}/logs/genpo_belm_genpo_multi_task_10seed_sweeps}"
...
Environment overrides:
  ...
  ALGORITHMS           Comma-separated algorithms. Default: genpo,belm_genpo
                       genpo maps to genpo; belmgenpo/belm_genpo map to belm_genpo.
  ...
  EXPERIMENT_PREFIX    Experiment name prefix. Default: genpo_belm_genpo_10seed
  WANDB_PROJECT_PREFIX Optional prefix for generated wandb projects.
                       Project format: <prefix_>task_slug_algorithm
  LAUNCH_LOG_DIR       Per-run log directory. Default: logs/genpo_belm_genpo_multi_task_10seed_sweeps
...
Examples:
  bash ${0##*/}
  GPU_IDS=0,1 SEEDS=0,1 TASKS=Isaac-Ant-v0,Isaac-Humanoid-v0 bash ${0##*/}
  ALGORITHMS=belm_genpo MAX_ITERATIONS=1000 WANDB_PROJECT_PREFIX=baseline bash ${0##*/}
EOF
}
...
task_slug() {
    ...
    case "${task}" in
        ...
        *)
            echo "Unsupported task for GenPO/BELM-GenPO sweep: ${task}" >&2
            exit 1
            ;;
    esac
}
...
canonical_algorithm() {
    ...
    case "${algorithm}" in
        ...
        *)
            echo "Unsupported algorithm '${algorithm}'. Use genpo or belm_genpo." >&2
            exit 1
            ;;
    esac
}

agent_entry_for_algorithm() {
    canonical_algorithm "$1"
}
...
build_wandb_project() {
    ...
    if [[ -n "${WANDB_PROJECT_PREFIX}" ]]; then
        echo "${WANDB_PROJECT_PREFIX}_${task_label}_${algorithm}"
    else
        echo "${task_label}_${algorithm}"
    fi
}
...
build_experiment_name() {
    ...
    echo "${prefix}${task_label}_${algorithm}_seed$(sanitize_value "${seed}")"
}
...
schedule_variant() {
    ...
    local wandb_project
    agent_entry="$(agent_entry_for_algorithm "${algorithm}")"
    experiment_name="$(build_experiment_name "${task_label}" "${algorithm}" "${seed}")"
    wandb_project="$(build_wandb_project "${task_label}" "${algorithm}")"

    schedule_run "${task}" "${agent_entry}" "${experiment_name}" "${wandb_project}" "${algorithm}" \
        "agent.seed=${seed}"
}
...
csv_to_array "${ALGORITHMS}" algorithms
csv_to_array "${SEEDS}" seeds
initialize_gpu_ids

if [[ ${#tasks[@]} -eq 0 || ${#algorithms[@]} -eq 0 || ${#seeds[@]} -eq 0 ]]; then
    echo "TASKS, ALGORITHMS, and SEEDS must all be non-empty." >&2
    exit 1
fi
...
for task in "${tasks[@]}"; do
    ...
    echo
    echo "==== Starting GenPO/BELM-GenPO sweep for ${task} (${task_label}) ===="

    for algorithm in "${algorithms[@]}"; do
        algorithm="$(trim_value "${algorithm}")"
        ...
        for seed in "${seeds[@]}"; do
            ...
            [[ -z "${seed}" ]] && continue
            schedule_variant "${task}" "${task_label}" "${algorithm}" "${seed}"
        done
    done
done
...
if [[ ${failed_jobs} -ne 0 ]]; then
    echo "GenPO/BELM-GenPO multi-task 10-seed sweep finished with ${failed_jobs} failed job(s)." >&2
    exit 1
fi
echo "GenPO/BELM-GenPO multi-task 10-seed sweep finished successfully."
